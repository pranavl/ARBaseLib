...
/**
 * An activity which can be subclassed to create an AR application. ARActivity
 * handles almost all of the required operations to create a simple augmented
 * reality application.
 *
 * ARActivity automatically creates a camera preview surface and an OpenGL
 * surface view, and arranges these correctly in the user interface.The subclass
 * simply needs to provide a FrameLayout object which will be populated with
 * these UI components, using {@link supplyFrameLayout}.
 *
 * To create a custom AR experience, the subclass should also provide a custom
 * renderer using {@link supplyRenderer}. This allows the subclass to handle
 * OpenGL drawing calls on its own.
 *
 */
public abstract class ARActivity extends Activity implements CameraEventListener {
    ...
    /**
     * Renderer to use. This is provided by the subclass using
     * {@link supplyRenderer()}.
     */
    protected ARRenderer renderer;

    /**
     * Layout that will be filled with the camera preview and GL views. This is
     * provided by the subclass using {@link supplyFrameLayout()}.
     */
