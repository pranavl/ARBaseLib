...
/**
 * The NativeInterface class contains the JNI function signatures for native
 * ARToolKit functions. These functions should be accessed via the
 * {@link ARToolKit} class rather than called directly.
 */
public class NativeInterface {
    ...
    /**
     * Attempts to load the native library so that native functions can be
     * called.
     *
    ...
    /**
     * Initialises the basic ARToolKit functions. After this function has been
     * successfully called, markers can be added and removed, but marker
    ...
    /**
     * Changes the working directory to the resources directory used by
     * ARToolKit. Normally, this would be called immediately after
     * arwInitialiseAR()
     *
    ...
    /**
     * Initialises stereo video capture. The native library will start to expect
     * video frames.
     *
     * @param vconfL	The video configuration string for the left camera. Can be
     * left empty.
     * @param cparaNameL	The camera parameter file to load for the left camera.
     * @param vconfR	The video configuration string for the right camera. Can be
     * left empty.
     ...
     *
     * @return	A float array containing the OpenGL compatible projection matrix,
     * or null if an error occurred.
     */
    public static native float[] arwGetProjectionMatrix();

    /**
     * Retrieves the ARToolKit projection matrix for the right camera of a
     * stereo camera pair.
     *
     * @return	A float array containing the OpenGL compatible projection matrix,
     * or null if an error occurred.
     ...
     *
     * Usage example: int[] width = new int[1]; int[] height = new int[1]; int[]
     * pixelSize = new int[1]; String[] pixelFormatString = new String[1];
     * boolean ok = NativeInterface.arwGetVideoParams(width, height, pixelSize,
     * pixelFormatString);
     *
     * @width An int array, the first element of which will be filled with the
     * width (in pixels) of the video frame, or null if this information is not
     * required.
     * @height An int array, the first element of which will be filled with the
     * height (in pixels) of the video frame, or null if this information is not
     * required.
     * @pixelSize An int array, the first element of which will be filled with
     * the numbers of bytes per pixel of the source frame, or null if this
     * information is not required.
     * @pixelFormatString A String array, the first element of which will be
     * filled with the symbolic name of the pixel format of the video frame, or
     * null if this information is not required. The name will be of the form
     * "AR_PIXEL_FORMAT_xxx".
     * @return	True if the values were returned OK, false if there is currently
     * no video source or an error int[] .
     ...
     *
     * Usage example: int[] widthL = new int[1]; int[] heightL = new int[1];
     * int[] pixelSizeL = new int[1]; String[] pixelFormatStringL = new
     * String[1]; int[] widthR = new int[1]; int[] heightR = new int[1]; int[]
     * pixelSizeR = new int[1]; String[] pixelFormatStringR = new String[1];
     * boolean ok = NativeInterface.arwGetVideoParams(widthL, heightL,
     * pixelSizeL, pixelFormatStringL, widthR, heightR, pixelSizeR,
     * pixelFormatStringR);
     *
     * @widthL An int array, the first element of which will be filled with the
     * width (in pixels) of the video frame, or null if this information is not
     * required.
     * @widthR An int array, the first element of which will be filled with the
     * width (in pixels) of the video frame, or null if this information is not
     * required.
     * @heightL An int array, the first element of which will be filled with the
     * height (in pixels) of the video frame, or null if this information is not
     * required.
     * @heightR An int array, the first element of which will be filled with the
     * height (in pixels) of the video frame, or null if this information is not
     * required.
     * @pixelSizeL An int array, the first element of which will be filled with
     * the numbers of bytes per pixel of the source frame, or null if this
     * information is not required.
     * @pixelSizeR An int array, the first element of which will be filled with
     * the numbers of bytes per pixel of the source frame, or null if this
     * information is not required.
     * @pixelFormatStringL A String array, the first element of which will be
     * filled with the symbolic name of the pixel format of the video frame, or
     * null if this information is not required. The name will be of the form
     * "AR_PIXEL_FORMAT_xxx".
     * @pixelFormatStringR A String array, the first element of which will be
     * filled with the symbolic name of the pixel format of the video frame, or
     * null if this information is not required. The name will be of the form
     * "AR_PIXEL_FORMAT_xxx".
     * @return	True if the values were returned OK, false if there is currently
     * no stereo video source or an error occurred.
     ...
     * @return	A unique identifier (UID) of the new marker, or -1 if the marker
     * was not added due to an error.
     */
     ...
     *
     * @param markerUID	The unique identifier (UID) of the marker to check
     * @return	A float array containing the OpenGL compatible transformation
     * matrix, or null if the marker isn't visible or an error occurred.
     */
     ...
     *
     * @param markerUID	The unique identifier (UID) of the marker to check
     * @param matrixL	A float array containing the OpenGL compatible
     * transformation matrix for the left camera.
     * @param matrixR	A float array containing the OpenGL compatible
     * transformation matrix for the right camera.
     * @return	true if the marker is currently visible, otherwise false.
     */
     ...
     * @param cameraIndex	Zero-based index of the camera in use. If only one
     * camera is present, will be 0.
     * @param cameraIsFrontFacing false if camera is rear-facing (the default)
     * or true if camera is facing toward the user.
     * @return	true if no error occurred, otherwise false
     */
    ...
    /**
     * Passes a video frame for the right camera of a stereo camera pair to the
     * native library for processing.
     *
     ...
     * @param cameraIndexL	Zero-based index of the camera in use. If only one
     * camera is present, will be 0.
     * @param cameraIsFrontFacingL false if camera is rear-facing (the default)
     * or true if camera is facing toward the user.
     ...
     * @param cameraIndexR	Zero-based index of the camera in use. If only one
     * camera is present, will be 0.
     * @param cameraIsFrontFacingR false if camera is rear-facing (the default)
     * or true if camera is facing toward the user.
     * @return	true if no error occurred, otherwise false
     */
